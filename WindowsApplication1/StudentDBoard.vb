...
Public Class StudentDBoard
    'Declaring variables
    Public Subjects(1) As String 'Array for storing SubjectIDs corresponding to subjects the student is taking
    ...
    Private Sub StudentDBoard_Load(sender As Object, e As EventArgs) Handles MyBase.Load 'Procedure for when form is loaded
        WhichForm = "StudentDBoard" 'Current form is StudentDBoard
        CheckAll() 'Making sure each database exists
        InitialiseDays() 'Calls procedure to initialise the Days array
        ...
        UpdateStudentTimetable() 'Calls procedure to update the student timetable
    End Sub

    Public Sub UpdateStudentTimetable() 'Declare procedure UpdateStudentTimetable
        If WhichForm <> "StudentDBoard" Then 'This makes sure that the procedure only runs when it is called
            Exit Sub
        End If
        ...
        For CurrentIndex = 0 To MaxStudentIndex 'Loops through each StudentID in the StudentIDs array
            If UserID = StudentIDs(CurrentIndex) Then 'If there is a match
                NameTimetable(lbName, "Students.txt", UserID) 'Obtains the Student's name to display on the timetable
